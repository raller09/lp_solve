...
uses
  ...
  Dialogs, ExtCtrls, StdCtrls, ComCtrls, Menus, SynEdit, ToolWin, SynMemo,
  SynEditTypes, LPHighlighter, ActnList, ImgList, XPMenu, SynEditSearch,
  LPSynEdit, SynEditRegexSearch, IniFiles, SynEditHighlighter, SynHighlighterXML,
  SynEditMiscClasses, Grids, VirtualTrees, lpsolve, SynEditExport, SynExportHTML,
  SynExportRTF, SynExportTeX, clipbrd, ResultArray, SynEditPlugins,
  SynMacroRecorder, Forms, lpobject; //, madExceptVcl, madexcept;
*)
uses
 ...
 // Windows ,
  ...
  Dialogs, ExtCtrls, StdCtrls, ComCtrls, Menus,  ToolWin, StrUtils,
  ActnList, ImgList, IniFiles, Grids, clipbrd,
  ResultArray, lpsolve, lpobject, VirtualTrees, VirtualStringTree ,
  sysutils, /// JwaWinUser, // Jwa for MessageBEEP
///  ModuleLoader, //  JWa for LoadModule
  dynlibs, // LoadLibrary for the xli_s
//   FileUtil ,  /// for SysToUTF8,UTF8ToSys  for synedit memos from callbacks ansi lp_solve55.dll  not needed in USA
   SynEdit,
   SynEditTypes,  SynEditMiscClasses, SynEditPlugins, SynEditSearch,
   SynMemo, SynEditHighlighter, SynHighlighterXML,
   SynMacroRecorder,  SynRegExpr, SynEditRegexSearch,
   SynEditExport, SynExportHTML,
  // SynExportRTF, SynExportTeX,
  LPHighlighter, LPSynedit
  ;
...
// event callbacks.
    procedure SolverResetCallbacks();
   {$IFDEF DELPHI6}
     procedure LPSolverLog(sender: TComponent; log: PChar);
   {$ELSE}
...
// end event callbacks
    ...
    procedure FormClose(Sender: TObject; var Action_close: TCloseAction);
    ...
    procedure EditorReplaceText(Sender: TObject; const ASearch,
      ...
      var Action_replace: TSynReplaceAction);
    ...
    procedure ObjectiveTableHeaderClick(Sender: TVTHeader;
    ...
    procedure ObjectiveTableBeforeCellPaint(Sender: TBaseVirtualTree;
    ...
    procedure ConstraintTableBeforeCellPaint(Sender: TBaseVirtualTree;
      TargetCanvas: TCanvas; Node: PVirtualNode; Column: TColumnIndex;
      CellRect: TRect);
    ...
    procedure ConstraintTableGetText(Sender: TBaseVirtualTree;
      Node: PVirtualNode; Column: TColumnIndex; TextType: TVSTTextType;
      var CellText: WideString);
    procedure ConstraintTableHeaderClick(Sender: TVTHeader;
      Column: TColumnIndex; Button: TMouseButton; Shift: TShiftState; X,
      Y: Integer);
    ...
    procedure MatrixTableInitNode(Sender: TBaseVirtualTree; ParentNode,
      Node: PVirtualNode; var InitialStates: TVirtualNodeInitStates);
    procedure MatrixTableHeaderClick(Sender: TVTHeader;
      Column: TColumnIndex; Button: TMouseButton; Shift: TShiftState; X,
      Y: Integer);
    procedure MatrixTableCompareNodes(Sender: TBaseVirtualTree; Node1,
      Node2: PVirtualNode; Column: TColumnIndex; var Result: Integer);
    procedure MatrixTableBeforeCellPaint(Sender: TBaseVirtualTree;
      TargetCanvas: TCanvas; Node: PVirtualNode; Column: TColumnIndex;
      CellRect: TRect);
    ...
    procedure ObjectiveSensTableGetText(Sender: TBaseVirtualTree;
      Node: PVirtualNode; Column: TColumnIndex; TextType: TVSTTextType;
      var CellText: WideString);
    procedure ObjectiveSensTableInitNode(Sender: TBaseVirtualTree;
      ParentNode, Node: PVirtualNode;
      var InitialStates: TVirtualNodeInitStates);
    procedure ObjectiveSensTableBeforeCellPaint(Sender: TBaseVirtualTree;
      TargetCanvas: TCanvas; Node: PVirtualNode; Column: TColumnIndex;
      CellRect: TRect);
    procedure ObjectiveSensTableHeaderClick(Sender: TVTHeader;
      Column: TColumnIndex; Button: TMouseButton; Shift: TShiftState; X,
      Y: Integer);
    ...
    procedure RHSSensTableGetText(Sender: TBaseVirtualTree;
      Node: PVirtualNode; Column: TColumnIndex; TextType: TVSTTextType;
      var CellText: WideString);
    procedure RHSSensTableBeforeCellPaint(Sender: TBaseVirtualTree;
      TargetCanvas: TCanvas; Node: PVirtualNode; Column: TColumnIndex;
      CellRect: TRect);
    procedure RHSSensTableHeaderClick(Sender: TVTHeader;
      Column: TColumnIndex; Button: TMouseButton; Shift: TShiftState; X,
      Y: Integer);
    ...
    procedure URLshow(UrlStr: string);
...
//      var handled: Boolean);
  ...
  public
    ...
    constructor Create(AOwner: TComponent; ALibName, AExtension: string; Lang: TLPLanguage); reintroduce;
  end;
...
implementation
uses dlgSearchText, dlgReplaceText, dlgConfirmReplace, dlgGotoLine,
  dlgStatistics, SynEditTextBuffer, dlgAbout, ShellAPI, Params;
...
constructor TNewXliExtension.Create(AOwner: TComponent; ALibName,
  AExtension: string; Lang: TLPLanguage);
var
  h: TLibHandle;
  m: TMenuItem;   AProc: Pointer;
  Cxli_name: string;  str: string;
begin
...
// dlopen in unix    fpc\2.2.2\source\rtl\unix\dl.pp(40)
// or jedi unit C:\Lazarus\fpc\2.2.2\source\packages\winunits-jedi\src\ModuleLoader;
///  h := LoadLibrary(PChar(FLibName));    // C:\Lazarus\fpc\2.2.2\source\rtl\win\sysutils.pp
  h := 0;  // required initializer (0)
  h := LoadLibrary( FLibName);
  if  h > 0 then
  begin
     assert(h <> 0);
///     caption := xli_name(GetProcAddress(h, 'xli_name'));
//??     if ModuleLoader.GetProcedure(const Cxli_name, AProc) then
     Aproc := GetProcedureAddress(h, Cxli_name);
     if Aproc <> nil then
     Begin
///        str := (xli_name(Aproc));     // this is a cast  Aproc a Pointer to xli_name = function: PChar; stdcall;
        caption := str;
 caption := ALibName; //'NewXliTest'; //  AnsiString(Lang);
        FreeLibrary(h);
...
constructor TViewXliExtension.Create(AOwner: TComponent; ALibName,
  AExtension: string; Lang: TLPLanguage);
var
  h: THandle;
  m: TMenuItem;
...
// {///
  ...
  assert(h <> 0);
//  caption := AnsiString(xli_name(GetProcAddress(h, 'xli_name')));   // this is a cast  Aproc to xli_name = function: PChar; stdcall;
 caption := ALibName; // 'ViewXliTest';  ///  AnsiString(Lang);
  FreeLibrary(h);
// }
...
// LPSolver Events
// *****************************************************************************
procedure Tmainform.SolverResetCallbacks();
begin
      LPSolver.OnLoad := (TNotifyEvent(@LPSolverLoad));
      LPSolver.OnAbort := (TOnAbort(@LPSolverAbort));
      LPSolver.OnLog := (TOnLog(@LPSolverLog));
      LPSolver.OnMessage := (TOnMessage(@LPSolverMessage));
end;

procedure Tmainform.LPSolverLog(sender: TComponent; log: PAnsiChar);
begin
//  MemoLog.Lines.Add(Trim(SysToUTF8(log)));   /// null action in USA whp
...
begin
  return := DoAbort;
  if (GetTickCount - FLastAbortEvent) > 50 then
  try
    if not DoAbort then
      Application.ProcessMessages;
    FLastAbortEvent := GetTickCount;
  except
    return := true;
    raise;
  end;
...
begin
  begin
    ...
    if (l = lXML) then
      Editor.Highlighter := SynXMLSyn else
      begin
    ...
    begin
      ...
      begin
        if SaveDialogScript.FilterIndex >= 3 then
          LPSolver.XLI := TNewXliExtension(XLINewActionList.Components[SaveDialogScript.FilterIndex - 3]).FLibName;
...
begin
  if (ActiveControl is TCustomSynEdit) then
    ...
    /// fails Format     also cntl-A not caught works in lazide source window.
end;
...
///  std_system('cls');
  MemoLog.ClearAll;
  MemoMessages.ClearAll;
  MemoLog.Lines.Add('Start Solve');
  MemoMessages.Lines.Add('Start Solve');
  MemoLog.CaretY := MemoLog.Lines.Count;
  MemoMessages.CaretY := MemoMessages.Lines.Count;
  ...
  begin
    ...
    try
      ...
      try
        ...
        if LPSolver.Solve in [stOptimal, stSubOptimal] then
...
var f: TGotoLineForm;  carret: TPoint;  line: integer;
begin
  PageControl.ActivePage := TabSheetEditor;
  f := TGotoLineForm.Create(self);
  try
   f.LineNumer.Text := IntToStr( {Mainform.} Editor.CaretY);
   line := Editor.CaretY;
    if f.ShowModal = mrOK then
    begin
 {     Editor.GotoLineAndCenter(StrToInt(f.LineNumer.Text));
  } /// inline from newer synedit
          carret := Point(1,StrToInt(f.LineNumer.Text));
 //         Editor.SetCaretXY(carret);    ///??
          Editor.LeftChar := carret.x;
          if (carret.y < Editor.TopLine) or (carret.y > (Editor.TopLine + (Editor.LinesInWindow - 1))) then
             Editor.TopLine := carret.y - (Editor.LinesInWindow - 1) div 2;
       Editor.SetFocus;
     end;
  finally
    f.Free;
...
begin
///  Clipboard.SetFormat(CF_HTML, ObjectiveTable.ContentToClipboard(CF_HTML, tstAll));
end;
...
begin
    ...
    for i := 0 to ControlCount - 1 do
      ...
      begin
        ...
        if (TAntiDegen(TCheckBox(Controls[i]).Tag) in def.AntiDegen) <> TCheckBox(Controls[i]).Checked then
          TCheckBox(Controls[i]).Font.Color := clRed else
          TCheckBox(Controls[i]).Font.Color := clDefault;
      end;
  ...
  if LPSolver.BoundsTighter <> def.BoundsTighter then
    BoundsTighterOption.Font.Color := clRed else
    BoundsTighterOption.Font.Color := clDefault;

  BreackAtFirstOption.Checked := LPSolver.BreakAtFirst;
  if LPSolver.BreakAtFirst <> def.BreakAtFirst then
    BreackAtFirstOption.Font.Color := clRed else
    BreackAtFirstOption.Font.Color := clDefault;
...
//          TCheckBox(Controls[i]).Font.Color := clDefault;
  ...
  if LPSolver.Debug <> def.Debug then
    DebugOption.Font.Color := clRed else
    DebugOption.Font.Color := clDefault;

  LagTraceOption.Checked := LPSolver.LagTrace;
  if LPSolver.LagTrace <> def.LagTrace then
    LagTraceOption.Font.Color := clRed else
    LagTraceOption.Font.Color := clDefault;

  TraceOption.Checked := LPSolver.Trace;
  if LPSolver.Trace <> def.Trace then
    TraceOption.Font.Color := clRed else
    TraceOption.Font.Color := clDefault;
  ...
  if LPSolver.FreeMPS <> def.FreeMPS then
    freeMPSOption.Font.Color := clRed else
    freeMPSOption.Font.Color := clDefault;
...
procedure TMainForm.SynMacroRecorderStateChange(Sender: TObject);
begin
...
procedure TMainForm.EditorStatusChange(Sender: TObject;
  Changes: TSynStatusChanges);
var carret: TPoint; /// TBufferCoord;
begin
  CheckGutter;
  carret := Editor.CaretXY;
//  StatusBar.Panels[0].Text := Format('%d:%d', [carret.Char, carret.Line]);
...
procedure TMainForm.ObjectiveTableGetText(Sender: TBaseVirtualTree;
  Node: PVirtualNode; Column: TColumnIndex; TextType: TVSTTextType;
  var CellText: WideString);
var
  row: integer;
...
procedure TMainForm.ObjectiveTableHeaderClick(Sender: TVTHeader;
  Column: TColumnIndex; Button: TMouseButton; Shift: TShiftState; X,
  Y: Integer);
begin
...
procedure TMainForm.ObjectiveTableCompareNodes(Sender: TBaseVirtualTree;
...
procedure TMainForm.ObjectiveTableInitNode(Sender: TBaseVirtualTree;
  ParentNode, Node: PVirtualNode;
  var InitialStates: TVirtualNodeInitStates);
begin
...
procedure TMainForm.ObjectiveTableBeforeCellPaint(Sender: TBaseVirtualTree;
  TargetCanvas: TCanvas; Node: PVirtualNode; Column: TColumnIndex;
  CellRect: TRect);
begin
  if node^.Index = 0 then
...
procedure TMainForm.FormClose(Sender: TObject; var Action_close: TCloseAction);
begin
...
var
  ...
  lps: TLPSolver;
begin
///  FormStyle := FSSPLASH;        //// whp  can see default screen built
  inherited;
  if LPSolver = nil then  LPSolver := TLPSolver.Create(nil);
  //MessageDlg(ExtractFilePath(ParamStr(0)) + 'default.lpo', mtError, [mbOK], 0);
...
var
  majorversion, minorversion_no, release_no, build_no: integer;
begin
  FCurrentFile := filename;
  lp_solve_version(@majorversion, @minorversion_no, @release_no, @build_no);
  if filename <> '' then
    Caption := format('LPSolve IDE - %d.%d.%d.%d - %s',[majorversion, minorversion_no,
      release_no, build_no, FCurrentFile]) else
    Caption := format('LPSolve IDE - %d.%d.%d.%d',[majorversion, minorversion_no,
      release_no, build_no]);
end;
...
begin
 ...
 ///     if Editor.SelAvail and (Editor.BlockBegin.Line = Editor.BlockEnd.Line) then
      if Editor.SelAvail and (Editor.BlockBegin.y = Editor.BlockEnd.y) then
        SearchText := Editor.SelText else
...
procedure TMainForm.EditorReplaceText(Sender: TObject; const ASearch,
  AReplace: String; Line, Column: Integer; var Action_replace: TSynReplaceAction);
var
  APos: TPoint;
  EditRect: TRect;
begin
  if ASearch = AReplace then
    Action_replace := raSkip
  else begin
 ...
 //       BufferCoord(Column, Line) ) ) );
    ...
    case ConfirmReplaceDialog.ShowModal of
      mrYes: Action_replace := raReplace;
      mrYesToAll: Action_replace := raReplaceAll;
      mrNo: Action_replace := raSkip;
      else Action_replace := raCancel;
    end;
  end;
end;
...
begin
  begin
    with TSynEditStringList(Editor.Lines) do
    begin
      fOnChange := OnChange;
...
{      fOnDeleted := OnDeleted;
 ...
 {??}         result := LPSolver.LoadFromFile('_tmpfile.txt',LPSolver.Verbose, Mode);
    finally
      with TSynEditStringList(Editor.Lines) do
      begin
        OnChange := fOnChange;
...
var
  ...
  ncol_1, ncol_Fobj: integer;   Pvars: PFloatArray;
begin
   if abs(LPSolver.Objective) <> LPSolver.Infinite then
   begin
     ncol_1 := LPSolver.NColumns+1;
     Pvars := FObjectives.Add(ncol_1);
 //    assert(LPSolver.GetVariables(FObjectives.Add(LPSolver.NColumns+1)));
     if  Not LPSolver.GetVariables(Pvars) then exit;
      ncol_Fobj :=  FObjectives.ColCount;
...
///     assert(LPSolver.GetConstraints(FConstraints.Add(LPSolver.NRows+1)));
     ncol_1 := LPSolver.NRows+1;
     Pvars := FConstraints.Add(ncol_1);
      if  Not LPSolver.GetConstraints(Pvars) then exit;
      ncol_Fobj :=  FConstraints.ColCount;
...
begin
///  Clipboard.SetFormat(CF_VRTF, ObjectiveTable.ContentToClipboard(CF_VRTF, tstAll));
// see procedure TIpHtml.CopyToClipboard
 Clipboard.Astext := String(ObjectiveTable.ContentToClipboard(CF_VRTF, tstAll));
end;

procedure TMainForm.CopyasCSV1Click(Sender: TObject);
begin
///  Clipboard.SetFormat(CF_CSV, ObjectiveTable.ContentToClipboard(CF_CSV, tstAll));
end;


procedure TMainForm.ConstraintTableBeforeCellPaint(
...
procedure TMainForm.ConstraintTableGetText(Sender: TBaseVirtualTree;
  Node: PVirtualNode; Column: TColumnIndex; TextType: TVSTTextType;
  var CellText: WideString);
var
  row: integer;
...
procedure TMainForm.ConstraintTableHeaderClick(Sender: TVTHeader;
  Column: TColumnIndex; Button: TMouseButton; Shift: TShiftState; X,
  Y: Integer);
begin
  if Button = mbLeft then
...
procedure TMainForm.MatrixTableGetText(Sender: TBaseVirtualTree;
  Node: PVirtualNode; Column: TColumnIndex; TextType: TVSTTextType;
  var CellText: WideString);
begin
  ...
  begin
    if Node^.Index > 0 then
    CellText := trim(LPSolver.RowName[PInteger(Sender.GetNodeData(Node))^]) else
    if LPSolver.Maximize then
      CellText := 'max' else
      CellText := 'min';
  end else
    if Column = LPSolver.NColumns + 1 then
      CellText := FloatToStr(LPSolver.Rh[PInteger(Sender.GetNodeData(Node))^]) else
      CellText := FloatToStr(LPSolver.Mat[PInteger(Sender.GetNodeData(Node))^, Column]);
end;

procedure TMainForm.MatrixTableInitNode(Sender: TBaseVirtualTree;
  ParentNode, Node: PVirtualNode;
  var InitialStates: TVirtualNodeInitStates);
begin
  PInteger(Sender.GetNodeData(Node))^ := Node^.Index;
...
procedure TMainForm.MatrixTableHeaderClick(Sender: TVTHeader;
  Column: TColumnIndex; Button: TMouseButton; Shift: TShiftState; X,
  Y: Integer);
begin
  if Button = mbLeft then
...
procedure TMainForm.MatrixTableBeforeCellPaint(Sender: TBaseVirtualTree;
  TargetCanvas: TCanvas; Node: PVirtualNode; Column: TColumnIndex;
  CellRect: TRect);
begin
  if node^.Index = 0 then
...
procedure TMainForm.ObjectiveSensTableBeforeCellPaint(
  Sender: TBaseVirtualTree; TargetCanvas: TCanvas; Node: PVirtualNode;
  Column: TColumnIndex; CellRect: TRect);
begin
  if Node^.Index = 0 then
...
procedure TMainForm.ObjectiveSensTableHeaderClick(Sender: TVTHeader;
  Column: TColumnIndex; Button: TMouseButton; Shift: TShiftState; X,
  Y: Integer);
begin
  if Button = mbLeft then
...
procedure TMainForm.ObjectiveSensTableCompareNodes(
...
procedure TMainForm.RHSSensTableInitNode(Sender: TBaseVirtualTree;
  ParentNode, Node: PVirtualNode;
  var InitialStates: TVirtualNodeInitStates);
begin
  PInteger(Sender.GetNodeData(Node))^ := Node^.Index;
...
procedure TMainForm.RHSSensTableBeforeCellPaint(Sender: TBaseVirtualTree;
  TargetCanvas: TCanvas; Node: PVirtualNode; Column: TColumnIndex;
  CellRect: TRect);
begin
  if Node^.Index = 0 then
...
procedure TMainForm.RHSSensTableHeaderClick(Sender: TVTHeader;
  Column: TColumnIndex; Button: TMouseButton; Shift: TShiftState; X,
  Y: Integer);
begin
  if Button = mbLeft then
...
procedure TMainForm.URLshow(UrlStr: string);
begin
...
begin
 /// SetThreadPriority(GetCurrentThread, pr[Priority.Position]);
  // \rtl\win\winevent.pp  built in on windows
end;
...
begin
  exceptIntf.BugReport := exceptIntf.BugReport + #13#10#13#10 + Editor.Text + #13#10#13#10 + MemoLog.Text;
  handled := false;
end;
*)
initialization
...
{$IFDEF LCL}
//{$I main.lrs}  {Include form's resource file}
{$ENDIF}
