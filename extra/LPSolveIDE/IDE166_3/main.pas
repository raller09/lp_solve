...
// , sysutils // LoadLibrary win32 for the xli_s
...
///  h := LoadLibrary(PChar(FLibName));    // C:\Lazarus\fpc\2.2.2\source\rtl\win\sysutils.pp
  h := INVALID_MODULEHANDLE_VALUE;  // required initializer (0)
  if LoadModule( h , FLibName) then
  begin
     assert(h <> 0);
///     caption := xli_name(GetProcAddress(h, 'xli_name'));
//??     if ModuleLoader.GetProcedure(const Cxli_name, AProc) then
     Begin
        caption := string(xli_name(Aproc));     // this is a cast  Aproc to xli_name = function: PChar; stdcall;
///     FreeLibrary(h);
        UnloadModule(h);
...
begin
  PageControl.ActivePage := TabSheetEditor;
  f := TGotoLineForm.Create(self);      /// whp something wrong here doesnot show
  try
 //   f.LineNumer.Text := '3';///  /IntToStr( {Mainform.} Editor.CaretY);
   line := Editor.CaretY;
///    f.LinenumberRealSetText('3');
