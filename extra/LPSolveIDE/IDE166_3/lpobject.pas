...
uses
  ...
  {$ENDIF}
  Classes, lpsolve ; //,   windows,dialogs;  // windows for threading   WaitForSingleObject
...
type
  ...
  TSolverStatus = (stUnknownError, stDataIgnored, stNoBfp, stNoMemory, stNotRun,
    ...
    stUserAbort, stTimeout, stRunning, stFutureStatus, stProcfail, stProcbreak,
...
//  TBBRule = packed record
//    case byte of
//    0: (select: TBBRuleSelect;
//        mode: TBBRuleModes;);
//    1: (code: integer;)
//  end;
  ...
  private
    ...
    FOnAbort: TOnAbort;
    FOnLog: TOnLog;
    FOnMessage: TOnMessage;    // function pointers
    ...
    function GetRh(const row: integer): double;
    function GetRhRange(const row: integer): double;
    procedure SetRh(const row: integer; const Value: double);
    procedure SetRhRange(const row: integer; const Value: double);
    function GetMat(const row, column: integer): double;
    procedure setMat(const row, column: integer; const Value: double);
    function GetBoundsTighter: boolean;
    function GetIsFree(const column: integer): boolean;
    function GetLowerBound(const column: integer): double;
    function GetUpperBound(const column: integer): double;
    procedure SetBoundsTighter(const Value: boolean);
    procedure SetIsFree(const column: integer; const Value: boolean);
    procedure SetLowerBound(const column: integer; const Value: double);
    procedure SetUpperBound(const column: integer; const Value: double);
    ...
    function getTimeElapsed: double;
    ...
    function ChangeLPHandle(newone: LP_handle): boolean;
    ...
    function GetObjBound: double;
    procedure SetObjBound(const Value: double);
    function GetMipGap(const absolute: boolean): double;
    procedure SetMipGap(const absolute: boolean; const Value: double);
    ...
    function GetEpsb: double;
    function GetEpsd: double;
    function GetEpsel: double;
    function GetInfinite: double;
    procedure SetEpsb(const Value: double);
    procedure SetEpsd(const Value: double);
    procedure SetEpsel(const Value: double);
    procedure SetInfinite(const Value: double);
    ...
    function GetScaleLimit: double;
    procedure SetScaleLimit(const Value: double);
    ...
    function GetBreakAtValue: double;
    procedure SetBreakAtValue(const Value: double);
    function GetEpsPerturb: double;
    function GetEpsPivot: double;
    function GetNegRange: double;
    procedure SetEpsPerturb(const Value: double);
    procedure SetEpsPivot(const Value: double);
    procedure SetNegRange(const Value: double);
    function GetMaxLevel: integer;
    function GetTotalIter: integer;
    function GetTotalNodes: integer;
    function GetObjective: double;
    function GetWorkingObjective: double;
    function GetVarDualResult(const index: integer): double;
    function GetVarPrimalResult(const index: integer): double;
    ...
    function GetEpsInt: double;
    procedure SetEpsInt(const Value: double);
  protected
    procedure DoOnload; virtual;
  public
    procedure Reset(const obCaller: Tobject);
    Procedure ResetParams;

    property Handle: LP_handle read FLP;
    ...
    // objective function
    function SetObj(Column: integer; Value: double): boolean;
    ...
    // Constraints
    function AddConstraint(row: PFloatArray; ConstrType: TConstrType; rh: double): boolean; overload;
    function AddConstraint(const RowString: string; ConstrType: TConstrType; rh: double): boolean; overload;
    function AddConstraintex(count: integer; row: PFloatArray; colno: PIntArray;
      ConstrType: TConstrType; rh: double): boolean;
    function GetRow(RowNr: integer; row: PFloatArray): boolean;
    function DelConstraint(DelRow: integer): boolean;

    // Lagrangian constraints
    function AddLagCon(row: PFloatArray; ConType: TConstrType; rhs: double): boolean; overload;
    function AddLagCon(const RowString: string; ConType: TConstrType; rhs: double): boolean; overload;

    // ConstrType
    property ConstrType[const row: integer]: TConstrType read GetConstrType write SetConstrType;

    property Rh[const row: integer]: double read GetRh write SetRh;
    property RhRange[const row: integer]: double read GetRhRange write SetRhRange;
    procedure SetRhVec(Prh: PFloatArray); overload;
    function SetRhVec(const Srh: string): boolean; overload;
    ...
    property Mat[const row, column: integer]: double read GetMat write setMat; default;

    // Set the upper and lower bounds of a variable
    function SetBounds(column: integer; lower, upper: double): boolean;
    property UpperBound[const column: integer]: double read GetUpperBound write SetUpperBound;
    property LowerBound[const column: integer]: double read GetLowerBound write SetLowerBound;
    ...
    // Add SOS constraints
    function AddSOS(const sosname: string; sostype: integer; priority: integer;
    ...
    function IsFeasible(Values: PFloatArray; threshold: double): boolean;
    function Solve: TSolverStatus;

    property TimeElapsed: double read getTimeElapsed;
    ...
    function LoadFromFile(filename: string; verbose: TVerbose; mode: TScriptFormat): boolean;
...
{    function LoadFromStream(stream: TStream; verbose: TVerbose; mode: TScriptFormat): boolean;
    ...
    property MipGap[const absolute: boolean]: double read GetMipGap write SetMipGap;
    ...
    property Objective: double read GetObjective;
    property WorkingObjective: double read GetWorkingObjective;

    property VarPrimalResult[const index: integer]: double read GetVarPrimalResult;
    property VarDualResult[const index: integer]: double read GetVarDualResult;
  ...
  published
    ...
    property ObjBound: double read GetObjBound write SetObjBound;
    property Infinite: double read GetInfinite write SetInfinite;
    property Epsb: double read GetEpsb write SetEpsb;
    property Epsd: double read GetEpsd write SetEpsd;
    property Epsel: double read GetEpsel write SetEpsel;
    property EpsPerturb: double read GetEpsPerturb write SetEpsPerturb;
    property EpsPivot: double read GetEpsPivot write SetEpsPivot;
    property EpsInt: double read GetEpsInt write SetEpsInt;

    property ScaleLimit: double read GetScaleLimit write SetScaleLimit;
    property BreakAtValue: double read GetBreakAtValue write SetBreakAtValue;
    property NegRange: double read GetNegRange write SetNegRange;
...
function std_system(p: PChar): integer; cdecl;  external MSVCRT name 'system';

//function std_mktemp(template: PChar): PChar;  cdecl; external MSVCRT name '_mktemp';
//function std_tmpfile: PInteger; cdecl;  external MSVCRT name 'tmpfile';
//function std_rmtmp: integer; cdecl;  external MSVCRT name '_rmtmp';
...
const
  ...
  function AbortHandler(lp: THandle; userhandle: Tcomponent): integer; stdcall;
  var abrt: boolean;
  begin
     abrt := false;
  ...
  procedure LogHandler(lp: THandle; userhandle: Tcomponent; buf: PChar); stdcall;
  begin
  ...
  procedure MessageHandler(lp: THandle; userhandle: Tcomponent; message: integer); stdcall;
  begin
...
function TLPSolver.SetObj(Column: integer; Value: double): boolean;
begin
...
function TLPSolver.AddConstraintex(count: integer; row: PFloatArray;
  colno: PIntArray; ConstrType: TConstrType; rh: double): boolean;
begin
...
function TLPSolver.AddLagCon(const RowString: string;
  ConType: TConstrType; rhs: double): boolean;
begin
...
function TLPSolver.GetRh(const row: integer): double;
begin
  result := get_rh(FLP, row);
end;

function TLPSolver.GetRhRange(const row: integer): double;
begin
  result := get_rh_range(FLP, row);
end;

procedure TLPSolver.SetRh(const row: integer; const Value: double);
begin
  set_rh(FLP, row, Value);
end;

procedure TLPSolver.SetRhRange(const row: integer; const Value: double);
begin
  set_rh_range(FLP, row, Value);
end;

procedure TLPSolver.SetRhVec(Prh: PFloatArray);
begin
  set_rh_vec(FLP, Prh);
end;

function TLPSolver.SetRhVec(const Srh: string): boolean;
begin
  result := str_set_rh_vec(FLP, PChar(Srh));
end;
...
function TLPSolver.GetMat(const row, column: integer): double;
begin
...
function TLPSolver.GetLowerBound(const column: integer): double;
begin
  result := get_lowbo(FLP, column);
end;

function TLPSolver.GetUpperBound(const column: integer): double;
begin
  result := get_upbo(FLP, column);
end;

function TLPSolver.SetBounds(column: integer; lower,
  upper: double): boolean;
begin
...
function TLPSolver.AddSOS(const sosname: string; sostype, priority,
  count: integer; sosvars: PIntArray; weights: PFloatArray): integer;
begin
  result := add_SOS(FLP, PChar(sosname), sostype, priority, count, sosvars, weights);
end;
...
begin
  set_BFP(FLP, Pointer(FBFP));
  result := TSolverStatus(lpsolve.solve(FLP) + 5);
end;

function TLPSolver.getTimeElapsed: double;
begin
...
function TLPSolver.LoadFromFile(filename: string; verbose: TVerbose;
  mode: TScriptFormat): boolean;
begin
  case mode of
    sfLP : result := ChangeLPHandle(read_lp(PChar(filename), ord(verbose), PChar(name)));
    sfMPS:
      begin
        if FFreeMPS then
          result := ChangeLPHandle(read_freeMPS(PChar(filename), ord(verbose))) else
          result := ChangeLPHandle(read_MPS(PChar(filename), ord(verbose)));
      end;
    sfXLI:
      begin
        result := ChangeLPHandle(read_XLI(PChar(FXLI), PChar(filename), Pointer(FXLIDataName), Pointer(FXLIOptions), ord(verbose)));
  ...
  if result then
           with MainForm do SolverResetCallbacks();
  DoOnload;
end;

{
function TLPSolver.LoadFromStream(stream: TStream; verbose: TVerbose;
  mode: TScriptFormat): boolean;
type
  ...
  begin
    ...
    case mode of
      ...
      sfMPS:
        begin
          if FFreeMPS then
            result := ChangeLPHandle(read_freeMPS(pout, ord(verbose))) else
            result := ChangeLPHandle(read_MPS(pout, ord(verbose)));
        end;
    end;
    if not result then
...
function TLPSolver.LoadFromStrings(strings: TStrings; verbose: TVerbose;
  mode: TScriptFormat): boolean;
{type
  PStreamDesc = ^TStreamDesc;
  TStreamDesc = record
  ...
  begin
    strm := std_fdopen(p.hndl, 'w');
    for i := 0 to p.str. Count - 1 do
      std_fputs(PChar(p.str[i]+#13#10), strm);
    std_fclose(strm);
    std_close(p.hndl);
    result := 1;
  end;
...
begin
  ...
  begin
    strings.SaveToFile(ExtractFilePath(ParamStr(0)) + tmpfile);
    result := LoadFromFile(ExtractFilePath(ParamStr(0)) + tmpfile, verbose, mode);
  end
;{ else
  if std_pipe(@Handles, 256, $8000) <> -1 then
  begin
...
function TLPSolver.SaveToFile(const filename: string;
  mode: TScriptFormat): boolean;
begin
  case mode of
    ...
    sfMPS:
      begin
        if FFreeMPS then
          result := write_freemps(FLP, PChar(filename)) else
...
type
  ...
  begin
    ...
    i := std_fgetc(p.pout);    ///whp  exception
    p.str.BeginUpdate;
    try
      while i > 0 do
      begin
...
begin
  ...
  if (mode = sfXLI) then
  begin
    result := SaveToFile(ExtractFilePath(ParamStr(0)) + tmpfile, mode);
    if result then
      strings.LoadFromFile(ExtractFilePath(ParamStr(0)) + tmpfile);
  end else
  if std_pipe(@Handles, 256, $8000) <> -1 then
  begin
    pin := std_fdopen(Handles[1], 'w');
...
function TLPSolver.ChangeLPHandle(newone: LP_handle): boolean;
var
...
function TLPSolver.GetObjBound: double;
begin
  if Maximize then
...
procedure TLPSolver.SetObjBound(const Value: double);
var
  v: double;
begin
  if Maximize then
...
function TLPSolver.GetMipGap(const absolute: boolean): double;
begin
...
function TLPSolver.GetEpsb: double;
begin
  result := get_epsb(FLP);
end;

function TLPSolver.GetEpsd: double;
begin
  result := get_epsd(FLP);
end;

function TLPSolver.GetEpsel: double;
begin
  result := get_epsel(FLP);
end;

function TLPSolver.GetInfinite: double;
begin
  result := get_infinite(FLP);
end;

procedure TLPSolver.SetEpsb(const Value: double);
begin
  if get_epsb(FLP) <> Value then
    set_epsb(FLP, Value);
end;

procedure TLPSolver.SetEpsd(const Value: double);
begin
  if get_epsd(FLP) <> Value then
    set_epsd(FLP, Value);
end;

procedure TLPSolver.SetEpsel(const Value: double);
begin
  if get_epsel(FLP) <> Value then
    set_epsel(FLP, Value);
end;

procedure TLPSolver.SetInfinite(const Value: double);
begin
...
function TLPSolver.GetScaleLimit: double;
begin
  result := get_scalelimit(FLP);
end;

procedure TLPSolver.SetScaleLimit(const Value: double);
begin
...
function TLPSolver.GetBreakAtValue: double;
begin
  if Maximize then
...
procedure TLPSolver.SetBreakAtValue(const Value: double);
var v: double;
begin
  if Maximize then
    v := -Value else
...
function TLPSolver.GetEpsPerturb: double;
begin
  result := get_epsperturb(FLP);
end;

function TLPSolver.GetEpsPivot: double;
begin
  result := get_epspivot(FLP);
end;

function TLPSolver.GetNegRange: double;
begin
  result := get_negrange(FLP);
end;

procedure TLPSolver.SetEpsPerturb(const Value: double);
begin
  if get_epsperturb(FLP) <> Value then
    set_epsperturb(FLP, Value);
end;

procedure TLPSolver.SetEpsPivot(const Value: double);
begin
  if get_epspivot(FLP) <> Value then
    set_epspivot(FLP, Value);
end;

procedure TLPSolver.SetNegRange(const Value: double);
begin
...
function TLPSolver.GetObjective: double;
begin
  result := get_objective(FLP)
end;

function TLPSolver.GetWorkingObjective: double;
begin
  result := get_working_objective(FLP)
end;

function TLPSolver.GetVarDualResult(const index: integer): double;
begin
  result := get_var_dualresult(FLP, index);
end;

function TLPSolver.GetVarPrimalResult(const index: integer): double;
begin
...
begin
 /// Stream.ReadComponent(Self);
  /// how to initialize ? @LogHandler etc  also Onload
  { in delphi main.dpr  no @ there
   object LPSolver: TLPSolver
    ...
    ...
    in Delphi
    main.TMainForm.LPSolverAbort
    main.TMainForm.LPSolverLog
    main.TMainForm.LPSolverMessage
    main.TMainForm.LPSolverLoad
  }
  /// done at reset
 { SetOnLog(FOnLog);
...
function TLPSolver.IsInfinite(const value: double): boolean;
begin
...
function TLPSolver.GetEpsInt: double;
begin
  result := get_epsint(FLP);
end;

procedure TLPSolver.SetEpsInt(const Value: double);
begin
  if get_epsint(FLP) <> Value then
    set_epsint(FLP, Value);
end;

procedure TLPSolver.Reset(const obCaller:Tobject);
begin
///
  if FLP <> 0 then delete_lp(FLP);
  FLP := make_lp(0,0);
//  SetOnAbort(FOnAbort);
//  SetOnLog(TOnLog(@obCaller.LPSolverLog));   ///(FonLog);
//  SetOnMessage(FOnMessage);
end;
