...
echo Compilation with VC6 does not work when linked with MySQL because of using long long.
...
%c% -I. -I.. -I../.. -I../../shared -I../../colamd -I%glpkdir%\include -I%glpkdir%\src -I"C:\Program Files\MySQL\MySQL Connector C 6.0.2\include" /LD /MD /Zp8 /Gz -D_WINDLL -D_USRDLL -D_CRT_SECURE_NO_DEPRECATE -D_CRT_NONSTDC_NO_DEPRECATE -DWIN32 -D__WOE__ -D_WINSOCKAPI_ -DNOMYSQL -DHAVE_CONFIG_H %src% ../lp_XLI.def -o bin\%PLATFORM%\xli_MathProg.dll
...
echo.
echo NOTE: Compilation with VC6 does not work when linked with MySQL so it is disabled in this DLL because of using long long.
