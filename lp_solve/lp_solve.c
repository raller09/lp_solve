...
void print_help(char *argv[])
{
	...
	*/
	...
	printf("-relax\t\tRelax model\n");
	printf("-relaxnotighten\tDon't tighten relaxed model\n");
	printf("-relaxexclude rowNr|RowName\n\t\tExclude row from relaxation\n");
	printf("-checkmultiplesolutions deltaobj deltax\n\t\tCheck if multiple solutions\n");
...
static void minmax(REAL value, REAL *minima, REAL *maxima, int *nminima, int *nmaxima, int row, int *rowmin, int *rowmax, int col, int *colindexMin, int *colindexMax)
{
	minmax1(value, -1.0, minima, nminima, row, rowmin, col, colindexMin);
	minmax1(value, +1.0, maxima, nmaxima, row, rowmax, col, colindexMax);
}
...
static void printminmax(lprec *lp, char *s, REAL *minima, REAL *maxima, int nminima, int nmaxima, int *rowmin, int *rowmax, int *colindexMin, int *colindexMax)
{
  printf("Minima:\n");
  printminmax1(lp, s, minima, nminima, rowmin, colindexMin);
  printf("Maxima:\n");
  printminmax1(lp, s, maxima, nmaxima, rowmax, colindexMax);
}
*/
...
static void printminmax(lprec *lp, char *s, REAL *minima, REAL *maxima, int nminima, int nmaxima, int *rowmin, int *rowmax, int *colindexMin, int *colindexMax)
{
	...
	for (i = 0; i < nminmax; i++) {
		...
		if (i < nminima) {
			...
			if ((rowmin != NULL) && (colindexMin != NULL))
				n += printf(", ");
			if (colindexMin != NULL)
				n += printf("%s", get_col_name(lp, colindexMin[i]));
		...
		if (i < nmaxima) {
			...
			if ((rowmax != NULL) && (colindexMax != NULL))
				n += printf(", ");
			if (colindexMax != NULL)
				n += printf("%s", get_col_name(lp, colindexMax[i]));
...
{
	REAL *col, *row, *RHSmin, *RHSmax, *OBJmin, *OBJmax, *MATmin, *MATmax, minValue, maxValue, a, *MATrowdiff, *MATrowmin, *MATrowmax;
	int *nz, ret, m, n, i, j, k, l, nRHSmin = 0, nRHSmax = 0, nOBJmin = 0, nOBJmax = 0, nMATmin = 0, nMATmax = 0, *rowRHSmin, *rowRHSmax, *colOBJmin, *colOBJmax, *rowMATmin, *rowMATmax, *colMATmin, *colMATmax, colindexMin, colindexMax, nMATrowmax = 0, *rowMATrowmax, *rowMATcolmin, *rowMATcolmax;
	...
	MATrowdiff = (REAL *)malloc(nstats * sizeof(*MATrowdiff));
	MATrowmin = (REAL *)malloc(nstats * sizeof(*MATrowmin));
	MATrowmax = (REAL *)malloc(nstats * sizeof(*MATrowmax));
	rowMATrowmax = (int *)malloc(nstats * sizeof(*rowMATrowmax));
	rowMATcolmin = (int *)malloc(nstats * sizeof(*rowMATcolmin));
	rowMATcolmax = (int *)malloc(nstats * sizeof(*rowMATcolmax));
	...
	*/
	...
	/* Matrix Coeficients per row */
	for (i = 1; i <= m; i++)
	{
		ret = get_rowex(lp, i, row, nz);
		minValue = DEF_INFINITE;
		maxValue = 0.0;
		colindexMin = colindexMax = -1;
		for (j = 0; j < ret; j++)
		{
			a = fabs(row[j]);
			if (a != 0.0)
			{
				...
				if (a > maxValue || (j != colindexMin && fabs(a - maxValue) < DEF_EPSVALUE))
				{
					maxValue = a;
					colindexMax = j;
				}
		...
		if (colindexMin != colindexMax)
		{
			/* a = maxValue - minValue; */ /* biggest difference */
			a = maxValue / minValue; /* biggest ratio */
			...
			if (k < nstats)
			{
				...
				if (l > 0)
				{
					...
					memmove(MATrowmin + k + 1, MATrowmin + k, l * sizeof(*MATrowmin));
					memmove(MATrowmax + k + 1, MATrowmax + k, l * sizeof(*MATrowmax));
					memmove(rowMATrowmax + k + 1, rowMATrowmax + k, l * sizeof(*rowMATrowmax));
					memmove(rowMATcolmin + k + 1, rowMATcolmin + k, l * sizeof(*rowMATcolmin));
					memmove(rowMATcolmax + k + 1, rowMATcolmax + k, l * sizeof(*rowMATcolmax));
				}

				if (nMATrowmax < nstats)
					nMATrowmax++;

				MATrowdiff[k] = a;
				MATrowmin[k] = row[colindexMin];
				MATrowmax[k] = row[colindexMax];
				rowMATrowmax[k] = i;
				rowMATcolmin[k] = nz[colindexMin];
				rowMATcolmax[k] = nz[colindexMax];
			}
		}
	}
	...
	printf("\nMatrix Coeficients per row (ratios): (");
	for (k = 0; k < nMATrowmax; k++)
		printf("%s%g", k > 0 ? ", " : "", MATrowdiff[k]);
	printf(")\n");
...
static void checkMultipleSolutions(lprec *lp, REAL multiplesolutionsdeltaobj, REAL multiplesolutionsdeltax)
{
	double o0, o, lowbo, upbo;
	int n, j, ret;
	char *name, found;
	...
	for (j = 1; j <= n; j++)
	{
		lowbo = get_lowbo(lp, j);
		upbo = get_upbo(lp, j);

		if (upbo - lowbo > multiplesolutionsdeltax)
		{
			found = FALSE;

			set_bounds(lp, j, lowbo + multiplesolutionsdeltax, lowbo + multiplesolutionsdeltax);
			ret = solve(lp);
			if (ret == 0)
			{
				o = get_objective(lp);
				if (fabs(o - o0) < multiplesolutionsdeltaobj)
				{
					name = get_col_name(lp, j);
					printf("Multiple solutions (%d - %s)\n", j, name);
					found = TRUE;
				}
			}

			if (!found && lowbo >= multiplesolutionsdeltax)
			{
				set_bounds(lp, j, lowbo - multiplesolutionsdeltax, lowbo - multiplesolutionsdeltax);
				ret = solve(lp);
				if (ret == 0)
			...
			set_bounds(lp, j, lowbo, upbo);
		}
	}
}

static int relax(int argc, char *argv[], lprec *lp)
{
	int result = INFEASIBLE, ret, m, n, i, j, cnt, *relaxed = NULL, *excludedRows = NULL, nExcludedRows = 0;
	REAL *obj = NULL, o, rhs, a, *var;
	char buf[2048], tighten = TRUE;

	printf("Adding relaxations...\n");

	m = get_Nrows(lp);
	n = get_Ncolumns(lp);

	excludedRows = (int *)malloc(m * sizeof(*excludedRows));

	for (i = 0; i < argc && nExcludedRows < m; i++)
	{
		if (strcmp(argv[i], "-relaxnotighten") == 0)
			tighten = FALSE;
		else if (strcmp(argv[i], "-relaxexclude") == 0)
		{
			i++;
			if (i < argc)
			{
				excludedRows[nExcludedRows] = atoi(argv[i]);
				if (excludedRows[nExcludedRows] == 0)
					excludedRows[nExcludedRows] = get_nameindex(lp, argv[i], TRUE);
				if (excludedRows[nExcludedRows] > 0)
					nExcludedRows++;
			}
		}
	...
	obj = (double *)malloc((1 + n + m * 2) * sizeof(*obj));
	if (obj == NULL)
		printf("Not enough memory\n");
	else
	{
		ret = get_row(lp, 0, obj);
		if (!ret)
			printf("get_row error (%d)\n", ret);
		else
		{
			o = 0.0;
			for (j = 1; j <= n; j++)
			...
			ret = resize_lp(lp, m, n + m * 2);
			if (!ret)
				printf("resize_lp error (%d)\n", ret);
			else
			{
				ret = TRUE;
				for (i = 1; i <= m && ret; i++)
				{
					...
					ret = add_columnex(lp, 1, &rhs, &i);
					if (!ret)
						printf("add_columnex error (%d)\n", ret);
					else
					{
						obj[n + (i - 1) * 2 + 1] = o;
						rhs = -rhs;
				...
				if (ret)
				{
					ret = set_obj_fn(lp, obj);
					if (!ret)
						printf("set_obj_fn error (%d)\n", ret);
					else
					{
						for (i = 0; i < nExcludedRows; i++)
						{
							ret = set_bounds(lp, n + (excludedRows[i] - 1) * 2 + 1, 0.0, 0.0);
							ret = set_bounds(lp, n + (excludedRows[i] - 1) * 2 + 2, 0.0, 0.0);
						}

						printf("Solving with relaxations ...\n");

						result = solve(lp);
						if (result)
							printf("Infeasible after adding relaxations\n");
						else
						{
							ret = get_ptr_variables(lp, &var);
							if (!ret)
								printf("get_ptr_variables error\n");
							else
							{
								relaxed = (int *)calloc(m, sizeof(*relaxed));
								if (relaxed == NULL)
									printf("Not enough memory\n");
								else
								{
									printf("Initial relaxations:\n");
									...
									for (j = 0; j < m; j++)
									{
										...
										if (a > 1e-5)
										{
											printf("Row %d (%s) relaxed (+%f)\n", j + 1, get_row_name(lp, j + 1), a * rhs);
											relaxed[cnt++] = j + 1;
										}
										else
										{
											...
											{
												ret = set_bounds(lp, n + j * 2 + 1, 0.0, 0.0);
												ret = set_bounds(lp, n + j * 2 + 2, 0.0, 0.0);
											}
										}
									}
									...
									if (tighten)
									{
										...
										for (i = 0; i < cnt; i++)
										{
											...
											ret = set_bounds(lp, n + (relaxed[i] - 1) * 2 + 2, 0.0, 0.0);
											...
											{
												/* infeasible */
												ret = set_bounds(lp, n + (relaxed[i] - 1) * 2 + 1, 0.0, DEF_INFINITE);
												ret = set_bounds(lp, n + (relaxed[i] - 1) * 2 + 2, 0.0, DEF_INFINITE);
											}
										}
										...
										if (tichtened)
										{
											...
											if (!result)
											{
												printf("Tightened up relaxations:\n");
												...
												for (i = 0; i < cnt; i++)
												{
													if (relaxed[i] != 0)
													{
														rhs = get_rh(lp, relaxed[i]);
														if (fabs(rhs) < 1e-5)
															rhs = 1.0;

														a = var[n + (relaxed[i] - 1) * 2];
														if (a > 1e-5)
														{
															printf("Row %d (%s) relaxed (+%f)\n", relaxed[i], get_row_name(lp, relaxed[i]), a * rhs);
														}

														a = var[n + (relaxed[i] - 1) * 2 + 1];
														if (a > 1e-5)
														{
															printf("Row %d (%s) relaxed (-%f)\n", relaxed[i], get_row_name(lp, relaxed[i]), a * rhs);
														}
...
int main(int argc, char *argv[])
{
	...
	REAL multiplesolutionsdeltaobj = 0.0, multiplesolutionsdeltax = 0.0;
	FILE *fpin = stdin;
	char *bfp = NULL;
...
# endif
	...
	for (i = 1; i < argc; i++) {
		...
		else if (strcmp(argv[i], "-Si") == 0)
		{
			...
			print_stats = TRUE;
		else if (strcmp(argv[i], "-checkmultiplesolutions") == 0 && (i + 2 < argc))
		{
			i++;
			multiplesolutionsdeltaobj = atof(argv[i]);
			i++;
			multiplesolutionsdeltax = atof(argv[i]);
		}
		else if (strcmp(argv[i], "-relax") == 0)
			relax_model = TRUE;
		else if (strcmp(argv[i], "-relaxnotighten") == 0)
			;
		else if (strcmp(argv[i], "-relaxexclude") == 0)
			i++;
	...
	if (relax_model)
		result = relax(argc, argv, lp);

	if (multiplesolutionsdeltaobj > 0.0 && multiplesolutionsdeltax > 0.0)
		checkMultipleSolutions(lp, multiplesolutionsdeltaobj, multiplesolutionsdeltax);
